...
start_api_server() {
    ...
    gnome-terminal -- bash -c "cd \"$current_dir/fastapi-server\" && python3 -m server; read -p 'Press Enter to exit' " &
}
...
# Add the current directory and the fastapi-server subfolder to PYTHONPATH
...
cd react-app || { log "Directory 'react-app' not found."; exit 1; }
